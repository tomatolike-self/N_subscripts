function plot_ionization_source_and_poloidal_stagnation_point_scd96data(all_radiationData, domain)
    % =========================================================================
    % 功能：
    ...
    for iDir = 1 : totalDirs
        ...
        % ------------------- 2) 计算电离源和极向速度 -------------------
        [ionSource2D, uPol2D] = compute_ionSource_and_uPol_2D(plasma_tmp, gmtry_tmp, dataStruct.dirName);
...
%% =========================================================================
function [ionSource2D, uPol2D] = compute_ionSource_and_uPol_2D(plasma_tmp, gmtry_tmp, caseDirPath)
    % 说明：
    %   - 电离源计算基于等离子体源项(sna)和密度(na)数据, 修改为只包含Ne1+
    ...
    neutsna_data = [];
    try
        % 获取算例目录路径已从函数参数传入 (caseDirPath)
        
        % 构建完整的电离源文件路径
        filename = fullfile(caseDirPath, 'scd96_SionNe0.txt');
        
        % 直接使用完整路径读取文件，跳过前3行头部
        data = importdata(filename, ' ', 3);
        
        % 提取数值数据
        numData = data.data;
        
        % 初始化电离源矩阵(nx-2 x ny-2) - 不包含guardcells的网格大小
        neutsna_data = zeros(nx-2, ny-2);
        ...
        for i = 1:size(numData, 1)
            ...
            if ix >= 1 && ix <= nx-2 && iy >= 1 && iy <= ny-2
                neutsna_data(ix, iy) = z01;
            end
        end
        
    catch ME
        warning(ME.identifier, '无法读取目录 %s 中的电离源数据文件: %s', caseDirPath, ME.message);
        % 如果读取失败，neutsna_data保持为空数组
    end

    % 只处理内部网格（排除guardcells）
    if ~isempty(neutsna_data)
        % 将电离源数据映射到ionSource2D(数据已经是归一化的，不需再除以体积)
        % 第一步：将原始非零值直接映射到结果矩阵
        for j = 1:ny-2
            for i = 1:nx-2
                if neutsna_data(i,j) ~= 0
                    ionSource2D(i,j) = neutsna_data(i,j);
                end
            end
        end
        
        % 第二步：按行处理零值网格 - 使用前面第一个非零值
        for j = 1:ny-2
            lastNonZeroValue = 0;  % 记录最后一个非零值
            lastNonZeroFound = false;  % 是否找到过非零值
            
            % 第一遍扫描：找出并填充零值网格
            for i = 1:nx-2
                if ionSource2D(i,j) ~= 0
                    % 记录非零值
                    lastNonZeroValue = ionSource2D(i,j);
                    lastNonZeroFound = true;
                elseif lastNonZeroFound
                    % 使用前面的非零值填充
                    ionSource2D(i,j) = lastNonZeroValue;
                end
            end
        ...
        % 第三步：检查仍然为零的网格（整行都是零的情况）
        zeroRows = [];
        for j = 1:ny-2
            if all(ionSource2D(1:nx-2,j) == 0)
                zeroRows = [zeroRows, j];
            end
        end
        
        % 如果有整行为零的，尝试使用相邻非零行的值
        if ~isempty(zeroRows)
            for k = 1:length(zeroRows)
                j = zeroRows(k);
                ...
                while ~nonZeroRowFound && searchRange < ny-2
                    % 检查上方行
                    if j-searchRange >= 1
                        upRow = j-searchRange;
                        if any(ionSource2D(1:nx-2,upRow) ~= 0)
                            % 复制上方非零行的值
                            ionSource2D(:,j) = ionSource2D(:,upRow);
                    ...
                    % 检查下方行
                    if j+searchRange <= ny-2
                        downRow = j+searchRange;
                        if any(ionSource2D(1:nx-2,downRow) ~= 0)
                            % 复制下方非零行的值
                            ionSource2D(:,j) = ionSource2D(:,downRow);
                            nonZeroRowFound = true;
                            break;
                        end
                    end
        ...
        % 最后检查，确保没有零值留下（如果整个数据都是零，则使用默认值）
        if all(ionSource2D(:) == 0)
            fprintf('警告：电离源数据全为零，使用默认值10^21填充\n');
            ionSource2D(:,:) = 1e21;
        end
    end
