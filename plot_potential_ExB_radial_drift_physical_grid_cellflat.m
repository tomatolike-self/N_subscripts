function plot_potential_ExB_radial_drift_physical_grid_cellflat(all_radiationData, domain, varargin)
% PLOT_POTENTIAL_EXB_RADIAL_DRIFT_PHYSICAL_GRID_CELLFLAT
%   基于物理网格绘制电势分布 + ExB径向漂移速度流型图（单元块颜色保持一致）。
...
%     'use_custom_colormap' (logical) - 是否加载 mycontour.mat 中的 colormap，默认 false。
    ...
    for i_case = 1:numel(all_radiationData)
        ...
        fprintf('Processing case for Potential & ExB radial drift (cell-flat color) on physical grid: %s\n', dirName);
        ...
        % 创建图形
        fig_title = sprintf('Potential & ExB Radial Drift (Physical Grid, Cell-Flat) - %s', dirName);
        ...
        saveFigureWithTimestamp(gcf, sprintf('Potential_ExB_RadialDrift_Physical_CellFlat_%s', createSafeFilename(dirName)));
        hold(ax, 'off');
    end

    fprintf('Potential & ExB radial drift (physical grid, cell-flat) plotting completed.\n');
end
