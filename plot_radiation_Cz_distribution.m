function plot_radiation_Cz_distribution(all_radiationData, domain)
    ...
    for iDir = 1:length(all_radiationData)
        ...
        impurity_density = sum(radInfo.plasma.na(:,:,3:13), 3);  % 杂质密度：Ne0到Ne10+的总和
        cz_ratio = impurity_density ./ radInfo.plasma.ne;  % 杂质浓度
    ...
    for iDir = 1:length(all_radiationData)
        ...
        if ~isempty(core_edge_indices)
            ...
            % Ne0粒子数
            Ne_core_edge_amount_Ne0 = sum(sum(radInfo.neut.dab(gmtry.leftcut+1:gmtry.rightcut,1:j_sep,2) .* radInfo.volcell(gmtry.leftcut+1:gmtry.rightcut,1:j_sep)));
            Ne_core_edge_amount = Ne_core_edge_amount + Ne_core_edge_amount_Ne0;
