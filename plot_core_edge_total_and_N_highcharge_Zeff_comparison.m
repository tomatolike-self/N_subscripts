...
if ~isempty(data_values) && ~all(isnan(data_values))
    ...
    % ----------------------------------------------------------------
    % 显示图例（使用 R2019a 兼容方式：创建 dummy patch 对象）
    % 注意：新版本 MATLAB 的 lgd.EntryContainer 属性在 R2019a 中不存在，
    % 因此采用创建不可见的 patch 对象来生成带颜色的图例条目
    % ----------------------------------------------------------------
    if showLegendsForDirNames && num_groups > 1
        ...
        % 创建 dummy patch 对象用于图例（R2019a 兼容方式）
        hold on;
        legend_handles = gobjects(num_groups, 1);
        for i_group = 1:num_groups
            % 创建一个不可见的 patch 对象，仅用于图例显示
            legend_handles(i_group) = patch(NaN, NaN, legend_colors(i_group, :), ...
                'EdgeColor', 'none', 'DisplayName', legend_entries{i_group});
        end
        hold off;
        
        % 使用 dummy patch 对象创建图例
        legend(legend_handles, legend_entries, 'Location', 'best', 'FontSize', fontSize-6);
    end
else
