function plot_N_ionization_rate_source_and_poloidal_stagnation_point(all_radiationData, domain, ionization_mode)
    ...
    %     1) 电离速率源分布 (使用 plasma.rsana(:,:,3:10) 数据) - **使用对数标尺显示**
    ...
    %       .plasma  (struct)   : 与 read_b2fplasmf 类似的等离子体数据(包含 na, fna_mdf, sna, rsana 等)
    %
    %   domain            : 用户选择的绘图区域 (0=全域,1=EAST上偏滤器,2=EAST下偏滤器)
    %   ionization_mode   : 电离速率模式 (0-7=特定价态，对应N0+到N7+)
    %
    % 依赖函数：
    %   - surfplot.m     (外部已存在的绘图函数)
    %   - plot3sep.m     (可选, 在图上叠加分离器/壁结构)
    %   - compute_ionizationRate_and_uPol_2D (本脚本中附带)
    %   - detect_poloidal_stagnation    (本脚本中附带)
    %   - computeCellCentersFromCorners (本脚本中附带)
    %   - saveFigureWithTimestamp       (本脚本中附带)
    %
    % 更新说明：
    %   - 基于 plot_Ne_ionization_rate_source_and_poloidal_stagnation_point.m 修改
    %   - 适用于N杂质体系：D(0价1，D+是2粒子编号) + N(从3到10，即N 0价到7价)
    %   - 使用 plasma.rsana(:,:,3:10) 作为电离速率数据源
