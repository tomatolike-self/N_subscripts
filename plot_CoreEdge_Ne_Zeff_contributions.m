function plot_CoreEdge_Ne_Zeff_contributions(all_radiationData, groupDirs, usePresetLegends, showLegendsForDirNames)
    % PLOT_COREEDGE_NE_ZEFF_CONTRIBUTIONS 绘制芯部边缘由各价态Ne离子分别贡献的Zeff数值（极向分布）
    ...
    fprintf('Creating Core Edge Ne Zeff contributions figure...\n');

    % --- 创建主图 (Core Edge Ne Ion Zeff Contributions) ---
    fig = figure('Name', sprintf('Core Edge Ne Ion Zeff Contributions (ix=%d) (All Groups, Color=Group)', core_edge_radial_index), ...
                  'Color', 'w', 'Position', [100 150 1200 800]);
    ax = struct();

    % 创建子图：Ne1+ 到 Ne10+ 的 Zeff 贡献
    for i_charge = 1:max_ne_charge
        subplot_idx = i_charge;
        if subplot_idx <= 10
            ax.(sprintf('ne%d', i_charge)) = subplot(3, 4, subplot_idx, 'Parent', fig);
            hold(ax.(sprintf('ne%d', i_charge)), 'on');
            title(ax.(sprintf('ne%d', i_charge)), sprintf('$Ne^{%d+}$ Zeff Contribution', i_charge), 'Interpreter', 'latex');
        end
    end

    % 创建D+子图
    ax.dplus = subplot(3, 4, 11, 'Parent', fig);
    hold(ax.dplus, 'on');
    title(ax.dplus, '$D^{+}$ Zeff Contribution', 'Interpreter', 'latex');

    % 创建除Ne8+外所有离子的总Zeff贡献子图（包括D+和其他Ne离子）
    ax.others = subplot(3, 4, 12, 'Parent', fig);
    hold(ax.others, 'on');
    title(ax.others, 'All Ions (except $Ne^{8+}$) Zeff Contribution', 'Interpreter', 'latex');
    ...
    for g = 1:numGroups
        ...
        for k = 1:numCasesInGroup
            ...
            % 计算各价态Ne离子对Zeff的贡献
            Zeff_contributions = zeros(ny, nx, max_ne_charge); % 只存储Ne1+到Ne10+的贡献

            num_Ne_species_in_b2 = size(nNe_all_charges_from_b2, 3);
            for i_Z = 2:min(num_Ne_species_in_b2, max_ne_charge + 1) % 从Ne1+开始 (i_Z=2对应Ne1+)
                charge_state = i_Z - 1; % i_Z=2 -> charge_state=1 (Ne1+)
                if charge_state >= 1 && charge_state <= max_ne_charge
                    Zeff_contributions(:,:,charge_state) = nNe_all_charges_from_b2(:,:,i_Z) * (charge_state^2) ./ safe_ne;
                end
            end
            ...
            % 提取芯部边界数据
            zeff_contributions_core_edge = squeeze(Zeff_contributions(:, core_edge_radial_index, :)); % [ny, max_ne_charge]
            zeff_D_contribution_core_edge = Zeff_D_contribution(:, core_edge_radial_index); % [ny, 1]

            % 计算除Ne8+外所有离子的总Zeff贡献（包括D+和其他Ne离子）
            ne8_charge_state = 8;
            ...
            % 添加除Ne8+外所有Ne离子的Zeff贡献
            for i_charge = 1:max_ne_charge
                if i_charge ~= ne8_charge_state % 排除Ne8+
            ...
            h_rep_candidate = [];
            for i_charge = 1:max_ne_charge
                ax_field = sprintf('ne%d', i_charge);
            ...
            % --- 绘图 (除Ne8+外所有离子的总Zeff贡献，包括D+和其他Ne离子) ---
    ...
    for i = 1:length(all_axes_flat)
        ...
        if i > 8 || current_ax == ax.dplus || current_ax == ax.others % 底部行的子图、D+子图和其他离子子图显示x轴标签
    ...
    % 保存图形
    savefig(fig, sprintf('CoreEdge_Ne_Zeff_Contributions_AllGroups_%s.fig', timestamp));

    fprintf('Core Edge Ne Zeff contributions plotting complete.\n');
end
