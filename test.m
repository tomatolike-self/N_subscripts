function test(all_radiationData, domain)
    % =========================================================================
    % 功能：
    %   针对每个算例(已在 all_radiationData 中存有 gmtry/plasma 等信息)，在 2D 平面上绘制：
    %     1) D+ 离子密度分布 (nDplus2D) - 使用对数标尺显示
    ...
    %   - compute_ionSource_and_parallelVelProj_and_nDplus_2D (本脚本中附带)
    ...
    %   - 背景色标修改为 D+ 离子密度 (对数标尺)
    %   - 颜色栏和标题更新以反映 D+ 离子密度
    %   - 其余功能与 plot_ionization_source_and_parallel_stagnation_point_D.m 保持一致
    % =========================================================================
    ...
    for iDir = 1 : totalDirs
        ...
        % ------------------- 2) 计算电离源, 平行速度极向投影, D+ 密度 -------------------
        [ionSource2D, parallelVelProj2D, nDplus2D] = compute_ionSource_and_parallelVelProj_and_nDplus_2D(plasma_tmp, gmtry_tmp);

        % ------------------- 3) 创建图窗并设置全局字体 -------------------
        figName = sprintf('D+ Density & Stagnation: %s', currentLabel);
        figure('Name', figName, 'NumberTitle','off', 'Color','w',...
               'DefaultAxesFontSize', fontSize, 'DefaultTextFontSize', fontSize);
        hold on;

        % (3.0) 对 D+ 密度数据取对数 (处理小于等于 0 的值)
        nDplus2D_log = log10(max(nDplus2D, eps)); % 使用 eps 避免 log10(0)

        % (3.1) 调用 surfplot 绘制 D+ 密度彩色图 (使用对数数据)
        surfplot(gmtry_tmp, nDplus2D_log);
        shading interp;
        view(2);
        colormap(jet);
        colorbarHandle = colorbar;

        % (3.2) 设置颜色栏标签及字体 (修改为对数标尺 D+ 密度)
        colorbarHandle.Label.String = 'log_{10}(n_{D+}) [log_{10}(m^{-3})]';
        ...
        title('Log_{10}(D+ Density) and Poloidal Stagnation','FontSize', fontSize+2); % 标题稍大
        ...
        % (3.6) 设置颜色映射的上下限 -  固定为 [22, 23] in log scale (可根据实际密度范围调整)
        caxis([19, 21]); % 调整 caxis 以适应 D+ 密度对数标尺范围
        ...
        for k = 1 : length(iPosVec)
            ...
            rr = rCenter(ii, jj);
            zz = zCenter(ii, jj);
            valZ = nDplus2D_log(ii, jj); % Z方向值取自**对数** D+ 密度数据
            plot3(rr, zz, valZ, 'o', 'Color', 'w', 'MarkerSize', markerSize);
        end

        % (5.3) 沿通量管连接停滞点，并根据平行速度极向投影正负值使用不同颜色
        connectStagnationPointsAlongFluxTubes(stagPoints, gmtry_tmp, rCenter, zCenter, nDplus2D_log, jSOLmin, jSOLmax, parallelVelProj2D); % 使用 nDplus2D_log 作为 zValues

        % (5.4) 添加固定的图例，只显示两个条目
        positiveVelColor = [0, 0.5, 0];    % 深绿色，高对比度
        negativeVelColor = [0.5, 0, 0.5];  % 深品红色，高对比度
        lineWidth = 1.5;                     % 线宽
        ...
        % ------------------- 6) 保存带时间戳的图窗 -------------------
        saveFigureWithTimestamp('Dplus_Density_Stagnation_logScale_parallelVelProj'); % 修改保存文件名以区分密度和速度类型

        hold off;
    end

    fprintf('\n>>> Completed: D+ density (log scale) and poloidal stagnation point distribution for all cases (D+ density, parallel vel. proj.).\n');

end % 主函数结束


%% =========================================================================
%% (A) 计算二维电离源, 平行速度极向投影, 和 D+ 密度分布 (子函数)
%% =========================================================================
function [ionSource2D, parallelVelProj2D, nDplus2D] = compute_ionSource_and_parallelVelProj_and_nDplus_2D(plasma_tmp, gmtry_tmp)
    % 说明：
    ...
    nDplus2D          = zeros(nx, ny);

    for jPos = 1 : ny
        for iPos = 1 : nx
            % ----- 电离源计算 (D+) -----
            gamma_Dplus = plasma_tmp.fna_mdf(iPos, jPos, 2);
            area_Dplus  = gmtry_tmp.gs(iPos, jPos, 1) * gmtry_tmp.qz(iPos, jPos, 2);
            ionSource2D(iPos, jPos) = gamma_Dplus / area_Dplus;
            ...
            % ----- D+ 密度 -----
            if size(plasma_tmp.na, 3) >= 2
                nDplus2D(iPos, jPos) = plasma_tmp.na(iPos, jPos, 2); % D+ 密度
            else
                nDplus2D(iPos, jPos) = 0; % 如果没有 D+ 组分，则密度为 0
            end
...
%% =========================================================================
%% (B) 检测极向速度停滞点 (修改以处理PFR区域) - 子函数 (保持不变)
%% =========================================================================
% ... (detect_poloidal_stagnation function - No changes needed)
...
%% (C) 计算网格中心坐标 (子函数保持不变)
%% =========================================================================
% ... (computeCellCentersFromCorners function - No changes needed)
...
%% (D) 带时间戳保存图窗 (子函数保持不变)
%% =========================================================================
% ... (saveFigureWithTimestamp function - No changes needed)
...
%% =========================================================================
%% (NEW) 沿通量管连接停滞点 - 改进版，支持PFR区域拼接网格 (子函数保持不变)
%% =========================================================================
% ... (connectStagnationPointsAlongFluxTubes function - No changes needed)
function connectStagnationPointsAlongFluxTubes(stagPoints, gmtry, rCenter, zCenter, zValues, jSOLmin, jSOLmax, parallelVelProj2D)
    % 说明：
    %   - 在每个通量管内，按照极向顺序连接停滞点
    %   - 沿通量管路径绘制连线，而非直线
...
%% =========================================================================
% ... (drawFluxTubeSegment function - No changes needed)
function drawFluxTubeSegment(iStart, iEnd, jChannel, rCenter, zCenter, zValues, color, width)
    ...
    % 确保z坐标值来自zValues（这里是nDplus2D_log）
    valZ_path = zValues(iPoints, jChannel);
