function plot_ionization_source_and_poloidal_stagnation_point_D(all_radiationData, domain)
    % =========================================================================
    % 功能：
    ...
    titleFontSize = 34;     % 标题字体大小
    markerSize = 8;         % 停滞点标记尺寸，增大标记尺寸
    % 定义线条颜色和线宽 - 移到全局参数，确保图例和实际绘图使用相同颜色
    positiveVelColor = [1.0, 0.5, 0.0];   % 明亮橙色
    negativeVelColor = [0.0, 0.9, 0.9];   % 鲜亮青色
    lineWidth = 2.0;                      % 增大线宽
    legendLineWidth = 3.0;                % 图例线宽稍粗，便于观察
    ...
    for iDir = 1 : totalDirs
        ...
        log_min = 22.5;
        log_max = 24.5;
        caxis([log_min, log_max]);
        
        % 创建自定义的colorbar
        cb = colorbar;
        
        % 创建5个均匀分布的刻度
        log_ticks = linspace(log_min, log_max, 5);
        
        % 提取最小和最大指数值
        base_exp = floor(log_min);  % 使用最小值的指数作为基准
        
        if isMATLAB2017b
            % MATLAB 2017b兼容方式设置colorbar
            
            % 计算对应的原始值系数（除以公共基数）
            scale_factor = 10^base_exp;
            real_ticks = 10.^log_ticks / scale_factor;
            
            % 设置刻度和标签 - 使用%.1f确保显示一位小数
            tick_labels = cell(length(log_ticks), 1);
            for i = 1:length(log_ticks)
                coeff = real_ticks(i);
                % 如果系数是整数或接近整数，不显示小数点
                if abs(coeff - round(coeff)) < 1e-10
                    tick_labels{i} = sprintf('%d', round(coeff));
                else
                    tick_labels{i} = sprintf('%.1f', coeff);
                end
            end
            
            % 应用到colorbar
            set(cb, 'Ticks', log_ticks, 'TickLabels', tick_labels, ...
                'FontName', 'Times New Roman', 'FontSize', fontSize-6);
            
            % 设置颜色栏标签
            ylabel(cb, 'Ionization Source [m^{-3}s^{-1}]', ...
                'FontName', 'Times New Roman', 'FontSize', fontSize-2, ...
                'FontWeight', 'bold');
            
            % 在colorbar上方添加指数部分
            title(cb, ['{\times}10^{', num2str(base_exp), '}'], ...
                'FontName', 'Times New Roman', 'FontSize', fontSize-2);
            ...
            cb.FontSize = fontSize-6;
            cb.LineWidth = 1.5;
            
            % 标签使用LaTeX解释器
            cb.Label.String = 'Ionization Source [m$^{-3}$s$^{-1}$]';
            cb.Label.Interpreter = 'latex';
            cb.Label.FontSize = fontSize-2;
            cb.Label.FontWeight = 'bold';
            
            % 在colorbar上方添加指数部分作为标题
            expStr = sprintf('$\\mathbf{\\times 10^{%d}}$', base_exp);
            title(cb, expStr, 'Interpreter', 'latex', 'FontSize', fontSize-2);
        end

        % (3.3) 叠加分离器/结构 (可选)
        plot3sep(gmtry_tmp, 'color','w','LineStyle','--','LineWidth',2.0);

        % (3.4) 设置坐标轴标签
        if isMATLAB2017b
            xlabel('R (m)', 'FontName', 'Times New Roman', 'FontSize', fontSize, 'FontWeight', 'bold');
            ylabel('Z (m)', 'FontName', 'Times New Roman', 'FontSize', fontSize, 'FontWeight', 'bold');
        else
        ...
        for k = 1 : length(iPosVec)
            ...
            plot3(rr, zz, valZ, 'o', 'Color', 'w', 'MarkerSize', markerSize, 'LineWidth', 1.8);
        end
        ...
        h1 = plot(NaN, NaN, '-', 'Color', positiveVelColor, 'LineWidth', legendLineWidth);
        h2 = plot(NaN, NaN, '-', 'Color', negativeVelColor, 'LineWidth', legendLineWidth);
        ...
        % ------------------- 6) 保存带时间戳的图窗 -------------------
        saveFigureWithTimestamp('IonSource_Stagnation_Dplus_logScale'); 

        hold off;
    end

    fprintf('\n>>> Completed: Ionization source (log scale) and poloidal stagnation point distribution for all cases (D+ ion source).\n');

end % 主函数结束


%% =========================================================================
%% (A) 计算二维电离源及极向速度分布 (子函数)
%% =========================================================================
function [ionSource2D, uPol2D] = compute_ionSource_and_uPol_2D(plasma_tmp, gmtry_tmp)
    ...
    for jPos = 1 : ny
        for iPos = 1 : nx
            ...
            % ----- 极向速度计算 -----
            gammaPol = plasma_tmp.fna_mdf(iPos, jPos, 1, 2); % 索引2为主离子 D+
            nD = plasma_tmp.na(iPos, jPos, 2);            % 主离子 D+ 密度
            if nD ~= 0
                uPol2D(iPos, jPos) = gammaPol / nD;
            else
                uPol2D(iPos, jPos) = 0;
            end
        end
...
function saveFigureWithTimestamp(baseName)
    ...
    try
        ...
        fprintf('警告: EPS格式保存失败，可能是MATLAB版本限制. 错误: %s\n', ME.message);
        % 尝试备用PDF格式，通常更兼容
        try
            pdfFile = sprintf('%s_%s.pdf', baseName, timestampStr);
            print(pdfFile, '-dpdf', '-painters', '-bestfit');
            fprintf('PDF矢量图已保存: %s (作为EPS的替代)\n', pdfFile);
        catch
            fprintf('警告: 所有矢量图格式保存均失败\n');
        end
    end
...
%% =========================================================================
%% (F) 辅助函数 - 提取科学计数法的系数和指数
%% =========================================================================
function [coefficient, exponent] = extractCoefficientAndExponent(number)
    % 将数值转换为科学计数法的系数和指数
    % 输入: number - 需要转换的数值
    % 输出: coefficient - 系数，通常为1.00-9.99
    %       exponent - 10的指数
