function exit_requested = select_and_execute_plotting_scripts_N(all_radiationData, groupDirs)
...
%
%   示例:
%     exit_requested = select_and_execute_plotting_scripts_N(all_radiationData, groupDirs)
...
while true % Start plotting script selection loop
    ...
    clear plot_main_sol_separatrix_radial_flux_distribution_N
    ...
    fprintf('78: Main SOL separatrix radial flux distribution (N1+ to N7+ charge states along poloidal grid)\n');
    ...
    if ismember(script_index, script_choices)
        ...
        fprintf('>>>                 Plotting N1+ to N7+ charge states radial flux along poloidal grid at separatrix\n');
        fprintf('>>>                 Main SOL separatrix position: poloidal grid 26-73, radial grid 14 (original grid)\n');
        
        % 选择绘图模式
        fprintf('\nPlease select plotting mode:\n');
        fprintf('  1: Individual charge states within a selected range (one subplot per charge state)\n');
        fprintf('  2: Summed charge state range (sum selected charge range)\n');
        plot_mode_choice = input('Please choose (1 or 2) [default=1]: ');
        
        if isempty(plot_mode_choice)
            plot_mode_choice = 1;
        end
        
        if plot_mode_choice == 2
            plot_mode = 'summed';
        else
        ...
        % 输入价态范围
        fprintf('\nPlease specify charge state range for plotting.\n');
        charge_range_input = input('Please enter charge state range (e.g., [1 7] for N1+ to N7+) [default=[1 7]]: ');
        if isempty(charge_range_input)
            charge_range = [1, 7];
        else
            charge_range = charge_range_input;
            if ~isnumeric(charge_range) || length(charge_range) ~= 2
                fprintf('Invalid input, using default range [1 7].\n');
                charge_range = [1, 7];
            else
                charge_range(1) = max(1, min(charge_range));
                charge_range(2) = min(7, max(charge_range));
                if charge_range(1) > charge_range(2)
                    fprintf('Invalid range, using default [1 7].\n');
                    charge_range = [1, 7];
                end
            end
        end
        
        fprintf('Using charge state range: N%d+ to N%d+\n', charge_range(1), charge_range(2));
        
        % 检查分组数量并选择图例/配色模式
        num_groups = length(groupDirs);
        if num_groups <= 1
            if num_groups == 0
                fprintf('No group information detected. Using single group color mode.\n');
            ...
            % 选择图例显示模式
            fprintf('\nPlease select legend display mode:\n');
            fprintf('  1: Simplified mode (show group colors only)\n');
            fprintf('  2: Full mode (show all case names)\n');
            legend_choice = input('Please choose (1 or 2) [default=1]: ');
            
            if isempty(legend_choice) || legend_choice == 1
                legend_mode = 'simplified';
                fprintf('Using simplified legend mode.\n');
            else
                legend_mode = 'full';
                fprintf('Using full legend mode.\n');
            end
        end
        
        % 调用主SOL分离面径向通量分布脚本函数
        plot_main_sol_separatrix_radial_flux_distribution_N(all_radiationData, groupDirs, ...
            'use_single_group_mode', use_single_group_mode, ...
            'plot_mode', plot_mode, ...
