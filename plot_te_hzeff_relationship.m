function plot_te_hzeff_relationship(all_radiationData, groupDirs, usePresetLegends, showLegendsForDirNames, useHardcodedLegends)
% PLOT_TE_HZEFF_RELATIONSHIP 绘制芯部电子温度与Hzeff关系图
%
%   此函数绘制芯部电子温度（Te）与辐射效率（Hzeff）的关系散点图
%   每个分组按照Te进行连线，形成点线图
%
%   参数:
...
%
%   电子温度计算方法：
%     Te_avg = sum(Te * ne * vol) / sum(ne * vol)
%     其中：
%     - Te: 电子温度 (eV)
%     - ne: 电子密度 (m^-3)
%     - vol: 体积 (m^3)
%     这是CEI处的电子温度按能量加权的极向平均值
%
%   辐射效率定义：
...
%
%   更新说明:
%     - 基于 plot_ne_hzeff_relationship.m 修改
%     - 横轴改为CEI处电子温度的能量加权极向平均值
%     - 只保留硬编码8组算例分组的模式3
%     - 支持MATLAB 2019a

    fprintf('\n=== Starting Core electron temperature vs Hzeff relationship analysis ===\n');
    
    % 检查输入参数
    if nargin < 5
        useHardcodedLegends = true; % 默认使用硬编码图例
    end
    if nargin < 4
    ...
    for i_case = 1:length(all_radiationData)
        ...
        fprintf('Processing case for Te-Hzeff analysis: %s\n', dirName);
        
        % 检查数据完整性
        can_process = true;
        if ~isfield(plasma, 'ne') || ~isfield(plasma, 'na') || ~isfield(gmtry, 'vol') || ~isfield(plasma, 'te')
        ...
        J_to_eV = 1.602e-19;
        if max(plasma.te(:)) > 1000  % 判断是否为J单位（通常J单位数值较大）
            plasma.te_ev = plasma.te / J_to_eV;
        else
            plasma.te_ev = plasma.te;  % 已经是eV单位
        end
        ...
        % 获取芯部电子密度和电子温度（CEI极向平均值）
        ne_core = plasma.ne(core_indices_original, 2); % 使用原始网格索引
        te_core = plasma.te_ev(core_indices_original, 2); % 使用原始网格索引，eV单位
        
        % 计算能量加权平均电子温度（CEI极向平均值）
        % Te_avg = sum(Te * ne * vol) / sum(ne * vol)
        ne_vol_sum = sum(ne_core .* core_vol, 'omitnan');
        
        if ne_vol_sum == 0 || isnan(ne_vol_sum)
            fprintf('Warning: Core electron count sum is zero or NaN for case %s. Skipping.\n', dirName);
            continue;
        end
        
        te_ne_vol_sum = sum(te_core .* ne_core .* core_vol, 'omitnan');
        core_avg_te = te_ne_vol_sum / ne_vol_sum;
        
        % 电子温度值（eV）
        te_value = core_avg_te;
        ...
        core_ne_for_zeff = plasma.ne(core_indices_original, 2);     % 获取芯部电子密度，使用原始网格索引

        % 计算 ne * 体积 的和 (用于后续加权平均)
        ne_vol_sum_zeff = sum(core_ne_for_zeff .* core_vol, 'omitnan');

        % 计算电子密度加权平均 Zeff
        if ne_vol_sum_zeff == 0 || isnan(ne_vol_sum_zeff)
            average_Zeff = NaN;
            ...
            Zeff_ne_vol_sum = sum(core_Zeff .* core_ne_for_zeff .* core_vol, 'omitnan'); % 计算 Zeff * ne * 体积 的和
            average_Zeff = Zeff_ne_vol_sum / ne_vol_sum_zeff; % 电子密度加权平均 Zeff
        end
        ...
        all_te_values(end+1) = te_value;
        all_hzeff_values(end+1) = hzeff_value;
        
        fprintf('  Te: %.3f eV, ne: %.3f (10^19 m^-3), Zeff: %.3f, P_rad: %.3f MW, Hzeff: %.3f\n', ...
                te_value, ne_value, average_Zeff, P_rad, hzeff_value);
    end
    
    fprintf('Successfully processed %d cases for Te-Hzeff analysis.\n', valid_cases);
    ...
    % 创建图形
    fig = figure('Name', 'Core electron temperature vs Hzeff relationship', 'NumberTitle', 'off', 'Color', 'w', ...
    ...
    % 绘制分组散点图
    plot_grouped_scatter_with_lines(all_dir_names, all_full_paths, all_te_values, all_hzeff_values, ...
                                   groupDirs, fontSize, linewidth, markerSize, ...
                                   usePresetLegends, showLegendsForDirNames, useHardcodedLegends);
    
    % 保存图形
    saveFigureWithTimestamp('Te_Hzeff_Relationship');
    
    fprintf('\n=== Core electron temperature vs Hzeff relationship analysis completed ===\n');
end

%% =========================================================================
%% 内部函数：绘制分组散点图（带连线）
%% =========================================================================
function plot_grouped_scatter_with_lines(dir_names, full_paths, te_values, hzeff_values, ...
                                        groupDirs, fontSize, linewidth, markerSize, ...
                                        usePresetLegends, showLegendsForDirNames, useHardcodedLegends)
    ...
    for i_group = 1:min(8, num_groups)
        ...
        % 提取当前组的数据
        group_te = te_values(group_mask);
        group_hzeff = hzeff_values(group_mask);

        % 移除NaN值
        valid_mask = ~isnan(group_te) & ~isnan(group_hzeff);
        group_te = group_te(valid_mask);
        group_hzeff = group_hzeff(valid_mask);

        if length(group_te) == 0
            continue; % 跳过没有有效数据的组
        end

        % 按Te值排序以便连线
        [group_te_sorted, sort_idx] = sort(group_te);
        ...
        % 绘制散点和连线
        h = plot(group_te_sorted, group_hzeff_sorted, '-', 'Color', current_color, ...
    ...
    % 设置坐标轴标签和属性
    xlabel('$T_e$ (eV)', 'FontSize', fontSize);
    ...
    % 设置坐标轴范围（可根据实际数据调整）
    % xlim([800, 1200]);  % 电子温度范围，可根据实际数据调整
    % ylim([0.5, 3.0]);   % Hzeff范围
