function plot_ionization_source_and_poloidal_stagnation_point_copy(all_radiationData, domain)
    % =========================================================================
    % 功能：
    ...
    % =========================================================================
        ...
        for iDir = 1 : totalDirs
            ...
            dataFileName = sprintf('IonSource2D_%s.dat',  timestampStr);
        ...
        for jPos = 1 : ny
            for iPos = 1 : nx
                ...
                if isfield(plasma_tmp, 'sna') && ndims(plasma_tmp.sna) >= 4
                    % 假设 sna(iPos,jPos,1,4)= coeff0; sna(iPos,jPos,2,4)= coeff1
                    %     plasma_tmp.na(iPos,jPos,4)= n4 (第四组分，Ne 1+)
                    if size(plasma_tmp.sna,3) >= 2 && size(plasma_tmp.sna,4) >= 4
                        coeff0 = plasma_tmp.sna(iPos, jPos, 1, 4);
                        coeff1 = plasma_tmp.sna(iPos, jPos, 2, 4);
    
                        if size(plasma_tmp.na,3) >= 4
                            n4 = plasma_tmp.na(iPos, jPos, 4);  % e.g. 第4组分
                        else
                            n4 = 0;
                        end
    
                        sVal = (coeff0 + coeff1*n4) * 1e-13;
                    end
                end
