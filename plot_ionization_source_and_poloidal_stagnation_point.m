function plot_ionization_source_and_poloidal_stagnation_point(all_radiationData, domain)
    ...
    for iDir = 1 : totalDirs
        ...
        % ------------------- 2) 计算电离源和极向速度 -------------------
        [ionSource2D, uPol2D] = compute_ionSource_and_uPol_2D(plasma_tmp, gmtry_tmp, neut_tmp);
...
%% =========================================================================
function [ionSource2D, uPol2D] = compute_ionSource_and_uPol_2D(plasma_tmp, gmtry_tmp, neut_tmp)
    % 说明：
        ...
        for iPos = 1 : nx
            ...
            if isfield(plasma_tmp, 'sna') && ndims(plasma_tmp.sna) >= 4
                ...
                for iComp = 4:13  % 对应Ne1+到Ne10+
                    ...
                    sVal = sVal + (coeff0_iComp + coeff1_iComp * n_iComp);
                end
            end
