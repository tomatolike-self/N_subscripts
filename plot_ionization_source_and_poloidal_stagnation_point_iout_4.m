function plot_ionization_source_and_poloidal_stagnation_point_iout_4(all_radiationData, domain)
    % =========================================================================
    % 功能：
...
function [ionSource2D, uPol2D] = compute_ionSource_and_uPol_2D(plasma_tmp, gmtry_tmp)
    ...
    try
        % 1. 读取文件，自动跳过第一行（标题行）
        full_neutsna_data = readmatrix('b2npc9_sna003.dat', 'NumHeaderLines', 1);
        
        % 2. 去掉第一列（行索引）
        neutsna_data = full_neutsna_data(:, 2:end);
        
        % 3. 转置矩阵从28x98变为98x28
        neutsna_data = neutsna_data.';
        
        % 4. 将所有数据的列顺序颠倒（从最后一列到第一列）
        neutsna_data = neutsna_data(:, end:-1:1);
    catch ME
        warning(ME.identifier, '无法读取电离源数据文件: %s', ME.message);
    ...
    for jPos = 1 : ny
        for iPos = 1 : nx
            % ----- 电离源计算 -----
            sVal = 0;
            if ~isempty(neutsna_data) && iPos <= size(neutsna_data, 1) && jPos <= size(neutsna_data, 2)
                sVal = neutsna_data(iPos, jPos);
            end
            
            ionSource2D(iPos, jPos) = sVal / gmtry_tmp.vol(iPos, jPos); % 单元体积归一化
