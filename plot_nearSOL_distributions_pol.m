function plot_nearSOL_distributions_pol(all_radiationData, radial_index_for_nearSOL, varargin)
    % =========================================================================
    % 功能：
...
function plot_nearSOL_distributions_fullDomain(all_radiationData, radial_index_for_nearSOL, legend_mode)
    ...
    % 线型定义：极向速度(u_pol)用实线'-'，电离源(S)用长虚线'--'，平行速度投影(parallelVelProj)用短虚线'-.'
    lineStyles_for_imp = {'-.', '-', '--'}; % 顺序对应 [parallelVelProj, u_pol, S]
    ...
    % ========== 设置坐标范围、轴标签等 ==========
    yyaxis left
    ylim([-1200,1600]); % 修改 Y 轴范围为 [-1200, 1600]
    yticks(-1200:400:1600); % 修改 Y 轴刻度，间隔为400
    ...
    yyaxis right
    ylim([-6,8]); % 等比例缩小为原来的一半
    yticks(-6:2:8); % 等比例调整刻度间隔
    ylabel('Ion Source ($10^{22}$ $\mathrm{m}^{-3}\mathrm{s}^{-1}$)', 'FontName', 'Times New Roman', 'FontSize', 32, 'FontWeight', 'bold', 'Interpreter', 'latex');
...
function [parallelVelProj, poloidalVelocity, ionSource] = ...
    ...
    for iPos=1:nxd_tmp
        ...
        if isfield(plasma_tmp,'sna') && ndims(plasma_tmp.sna)>=4
            ...
            if is <= size(plasma_tmp.na, 3) && is <= size(plasma_tmp.sna, 4) % 检查索引是否有效
                % 提取系数和密度
