function all_cases = get_all_case_data_N()
% ========================================================================
% 获取所有N杂质算例数据的函数
% ========================================================================
%
...
try
    ...
    error('Failed to load predefined N case groups: %s', ME.message);
end
...
% 处理fav BT组
fprintf('Processing fav BT groups...\n');
for n_idx = 1:length(n_concentrations)
    n_conc = n_concentrations(n_idx);
    if n_idx <= length(predefined_groups.fav_BT)
        group_paths = predefined_groups.fav_BT{n_idx};
        for path_idx = 1:length(group_paths)
            case_path = group_paths{path_idx};
            
            % 从路径中提取功率信息（假设路径包含功率信息）
...
% 处理unfav BT组
fprintf('Processing unfav BT groups...\n');
for n_idx = 1:length(n_concentrations)
    n_conc = n_concentrations(n_idx);
    if n_idx <= length(predefined_groups.unfav_BT)
        group_paths = predefined_groups.unfav_BT{n_idx};
        for path_idx = 1:length(group_paths)
            case_path = group_paths{path_idx};
...
fprintf('Total N impurity cases loaded: %d\n', length(all_cases));
